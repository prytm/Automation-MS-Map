...
with st.sidebar:
    ...
    page = st.radio(
        "Menu",
        ["Proses", "Manual Format Data", "Tentang"],
        index=0
    )
    st.markdown("---")
    st.caption("Tips cepat:")
    st.caption("• Kolom terakhir diberi 1 kolom kosong/“-”")
    st.caption("• Di bawah data beri 2 baris kosong atau 'CATATAN'")

# === HALAMAN: MANUAL FORMAT DATA ===
def render_manual():
    st.header("Manual • Format Data Bulan Ini")
    st.markdown("""
**Struktur header & data (baris/row):**
- **Row 6** → **Produsen** (per kolom)
- **Row 7** → **Kemasan** *(isi: Bag / Bulk; “Curah” otomatis dianggap Bulk)*
- **Row 52** → **Merk**
- **Row 53** → **Holding**
- **Row 8 → bawah** → data per **Provinsi/Daerah**

**Kolom Provinsi/Daerah:**
- Header kolom Provinsi harus mengandung kata **“Provinsi”** (boleh di row 6/7/52).
- Data provinsi mulai baris 8 (Aceh, Sumut, dst).

**Sinyal berhenti (agar unpivot tahu batas):**
- **Stop kanan**: jika **sel baris 8** pada kolom tsb **kosong** atau berisi **“-”**.
- **Stop bawah**: jika:
  - Baris provinsi berisi teks yang diawali **“CATATAN”**, **atau**
  - Ada **2 baris kosong berturut-turut** di kolom Provinsi.
- Baris subtotal yang diawali **“TOTAL …”** akan **di-skip**.

**Nilai data:**
- Boleh kosong atau “-” (dibaca 0). Angka dengan titik/koma ribuan aman (dibersihkan).
- Hindari teks non-angka di area data (selain “-”); jika ada, akan dianggap 0.

**Merge header:**
- Boleh merge untuk header; pastikan nilai teks ada di **sel kiri-atas** area merge.

**Konsistensi penamaan:**
- Gunakan ejaan konsisten untuk **Produsen / Merk / Holding / Kemasan**.
""")

    with st.expander("Contoh sketsa layout (ringkas)", expanded=False):
        st.markdown("""
**Row 6 (Produsen):** `Provinsi | PT. A | PT. A | PT. B | PT. B | …`  
**Row 7 (Kemasan):**  `Provinsi | Bag  | Bulk | Bag  | Bulk | …`  
**Row 52 (Merk):**    `Provinsi | M1   | M1   | M2   | M2   | …`  
**Row 53 (Holding):** `Provinsi | H1   | H1   | H2   | H2   | …`  
**Row 8+ (Data):**    `Aceh, Sumut, …` dengan angka pada sel-sel di bawah kolom-kolom di atas.

→ Setelah kolom terakhir, beri **1 kolom kosong/“-”**.  
→ Setelah baris data terakhir, beri **2 baris kosong** atau baris **“CATATAN:”**.
        """)

    st.subheader("Checklist Sebelum Proses")
    st.markdown("""
- [ ] Header **Produsen (6)**, **Kemasan (7)**, **Merk (52)**, **Holding (53)** sudah terisi
- [ ] Kolom **Provinsi** terdeteksi (ada teks “Provinsi” di salah satu header 6/7/52)
- [ ] Ada **pembatas kanan** (Row 8 kosong/“-” pada kolom setelah kolom terakhir data)
- [ ] Ada **pembatas bawah** (dua baris kosong atau baris **CATATAN**)
- [ ] Baris **TOTAL …** ada jika pakai subtotal (akan otomatis di-skip)
- [ ] Penamaan konsisten (Produsen/Merk/Holding/Kemasan)
- [ ] Data numerik tertulis standar; “-” atau kosong = 0
    """)
...
def render_about():
    st.header("Tentang")
    st.markdown("""
Aplikasi ini melakukan *unpivot* data penjualan per **Provinsi × Produsen × Kemasan**,
dengan **Merk** dan **Holding** diambil dari baris 52–53 yang sejajar kolom.  
Output akhir: `Daerah, Kemasan, Produsen, Holding, Merk, Total` + metrik MS & Growth.
    """)

# === ROUTING HALAMAN ===
if page == "Manual Format Data":
    render_manual()
elif page == "Tentang":
    render_about()
else:
    # === HALAMAN PROSES (taruh UI/logic lama kamu di sini) ===
    # Contoh: panggil fungsi/komponen proses yang sudah kamu tulis sebelumnya
    st.header("Proses Data")
    st.write("Silakan upload & jalankan proses seperti biasa dari halaman ini.")
